...
/* rtc_WavReader* rtc_WavOpen(const char* filename) {
  return reinterpret_cast<rtc_WavReader*>(
      new webrtc::WavReader(filename));
}

void rtc_WavClose(rtc_WavReader* wr) {
  delete reinterpret_cast<webrtc::WavReader*>(wr);
}

void rtc_WavReadSamples(rtc_WavReader* wr,
                        float* samples,
                        size_t *num_samples) {
  RTC_CHECK(num_samples);
  *num_samples = 
    reinterpret_cast<webrtc::WavReader*>(wr)->ReadSamples(samples, *num_samples);
}

int rtc_WavSampleRate(const rtc_WavReader* wr) {
  return reinterpret_cast<const webrtc::WavReader*>(wr)->sample_rate();
}

size_t rtc_WavNumChannels(const rtc_WavReader* wr) {
  return reinterpret_cast<const webrtc::WavReader*>(wr)->num_channels();
}

size_t rtc_WavNumSamples(const rtc_WavReader* wr) {
  return reinterpret_cast<const webrtc::WavReader*>(wr)->num_samples();
} */
