...
void IdentifySmallNarrowBandRegions(
    ...
    RTC_VIEW(size_t) /* kFftLengthBy2 - 1 */ narrow_band_counters) {
  ...
  size_t channel_counters[kFftLengthBy2 - 1];
  RTC_VIEW(size_t) channel_counters_view = RTC_MAKE_VIEW(size_t)(channel_counters);
  channel_counters_view.fill(0);
  ...
  for (size_t ch = 0; ch < X2.size(); ++ch) {
    for (size_t k = 1; k < kFftLengthBy2; ++k) {
      if (X2[ch][k] > 3 * std::max(X2[ch][k - 1], X2[ch][k + 1])) {
        ++channel_counters_view[k - 1];
      }
    }
  }
  for (size_t k = 1; k < kFftLengthBy2; ++k) {
    narrow_band_counters[k - 1] =
        channel_counters_view[k - 1] > 0 ? narrow_band_counters[k - 1] + 1 : 0;
  }
}
...
void IdentifyStrongNarrowBandComponent(const RenderBuffer& render_buffer,
  ...
  for (size_t channel = 0; channel < x_latest[0].size(); ++channel) {
    /* auto& */ RTC_VIEW(const float) /* kFftLengthBy2Plus1 */ X2_latest = 
        X2[channel];
...
RenderSignalAnalyzer::RenderSignalAnalyzer(const EchoCanceller3Config& config)
  ...
  narrow_band_counters_view_.fill(0);
}
...
void RenderSignalAnalyzer::MaskRegionsAroundNarrowBands(
  ...
  if (narrow_band_counters_view_[0] > kCounterThreshold) {
    v[1] = v[0] = 0.f;
  }
  for (size_t k = 2; k < kFftLengthBy2 - 1; ++k) {
    if (narrow_band_counters_view_[k - 1] > kCounterThreshold) {
      v[k - 2] = v[k - 1] = v[k] = v[k + 1] = v[k + 2] = 0.f;
    }
  }
  if (narrow_band_counters_view_[kFftLengthBy2 - 2] > kCounterThreshold) {
