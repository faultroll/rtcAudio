...
// namespace webrtc {
using BlockMeanCalculator = webrtc::BlockMeanCalculator;
...
void BlockBuffer::ExtractExtendedBlock(float extended_block[PART_LEN2]) {
  ...
  RTC_DCHECK_LT((size_t)0, AvaliableSpace());
...
static void UpdateLogRatioMetric(Stats* metric,
  ...
  RTC_CHECK_NE((size_t)0, metric->counter);
  ...
  if (metric->instant > metric->average) {
    metric->hicounter++;
    // This is to protect overflow, which should almost never happen.
    RTC_CHECK_NE((size_t)0, metric->hicounter);
...
AecCore* WebRtcAec_CreateAec(int instance_count) {
  ...
  aec->data_dumper.reset(new ApmDataDumper(instance_count));
  ...
  // assume not all functions are optimized
  extern void WebRtcAec_InitAec_c(void);
  WebRtcAec_InitAec_c();

#if defined(WEBRTC_HAS_SSE2) // #if defined(WEBRTC_ARCH_X86_FAMILY)
// if (webrtc::GetCPUInfo(kSSE2)) {
  extern void WebRtcAec_InitAec_SSE2(void);
  WebRtcAec_InitAec_SSE2();
// }
// #endif

#elif defined(MIPS_FPU_LE)
  extern void WebRtcAec_InitAec_mips(void);
  WebRtcAec_InitAec_mips();

#elif defined(WEBRTC_HAS_NEON)
  extern void WebRtcAec_InitAec_neon(void);
...
void FormNearendBlock(
    ...
    float nearend_block[NUM_HIGH_BANDS_MAX + 1][PART_LEN]) {
  RTC_DCHECK_LE(num_samples_from_nearend_frame, (size_t)PART_LEN);
...
void FormOutputFrame(size_t output_start_index,
  ...
  RTC_DCHECK_LE((size_t)FRAME_LEN, *output_buffer_size);
  for (size_t i = 0; i < num_bands; ++i) {
  ...
  if (*output_buffer_size > 0) {
    RTC_DCHECK_GE((size_t)(2 * PART_LEN - FRAME_LEN), (*output_buffer_size));
    for (size_t i = 0; i < num_bands; ++i) {
