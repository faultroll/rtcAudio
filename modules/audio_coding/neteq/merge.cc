...
size_t Merge::Process(int16_t* input,
  ...
  for (size_t channel = 0; channel < num_channels_; ++channel) {
    ...
    *external_mute_factor =
    ...
    RTC_DCHECK_GE(*external_mute_factor, 0);
    ...
    if (*external_mute_factor < 16384) {
      ...
      const int back_to_fullscale_inc = static_cast<int>(
          ((16384 - *external_mute_factor) << 6) / input_length_per_channel);
      const int increment = std::max(4194 / fs_mult_, back_to_fullscale_inc);
      *external_mute_factor = static_cast<int16_t>(DspHelper::RampSignal(
          input_channel.get(), interpolation_length, *external_mute_factor, increment));
...
int16_t Merge::SignalScaling(const int16_t* input,
  ...
  const size_t mod_input_length = rtc::SafeMin<size_t>(
