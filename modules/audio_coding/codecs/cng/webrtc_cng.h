...
class ComfortNoiseDecoder {
 public:
  ...
  void UpdateSid(RTC_VIEW(const uint8_t) sid);
  ...
  bool Generate(RTC_VIEW(int16_t) out_data, bool new_period);
 ...
 private:
  ...
  RTC_DISALLOW_COPY_AND_ASSIGN(ComfortNoiseDecoder);
};
...
class ComfortNoiseEncoder {
 public:
  ...
  size_t Encode(RTC_VIEW(const int16_t) speech,
                bool force_sid,
                rtc::Buffer* output);
 ...
 private:
  ...
  RTC_DISALLOW_COPY_AND_ASSIGN(ComfortNoiseEncoder);
};
