...
void ComfortNoiseDecoder::UpdateSid(RTC_VIEW(const uint8_t) sid) {
...
bool ComfortNoiseDecoder::Generate(RTC_VIEW(int16_t) out_data,
                                   bool new_period) {
...
size_t ComfortNoiseEncoder::Encode(RTC_VIEW(const int16_t) speech,
                                   bool force_sid,
                                   rtc::Buffer* output) {
  ...
  if ((enc_msSinceSid_ > (enc_interval_ - 1)) || force_sid) {
    ...
    output->AppendData(output_coefs, [&](RTC_VIEW(uint8_t) output) {
